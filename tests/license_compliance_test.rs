...
fn test_cargo_deny_config_exists() {
    ...
    let content = fs::read_to_string(config_path).expect("Should be able to read deny.toml");
...
fn test_allowed_licenses_configuration() {
    ...
    let allowed_licenses: Vec<&str> = allow.iter().map(|v| v.as_str().unwrap()).collect();
    ...
    assert!(
        allowed_licenses.contains(&"BSD-3-Clause"),
        "BSD-3-Clause license should be allowed"
    );
    assert!(
        allowed_licenses.contains(&"ISC"),
        "ISC license should be allowed"
    );
}
